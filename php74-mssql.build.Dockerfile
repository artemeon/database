...
RUN apt-get update \
  && apt-get install -y git libldap2-dev libxslt-dev zlib1g-dev libzip-dev libfreetype6-dev libjpeg62-turbo-dev libpng-dev libpq-dev zip unzip ant libaio1 gnupg2 \
  && docker-php-ext-install exif gd opcache sockets xsl zip intl bcmath calendar ldap \
